...
const Stake2Card = ({ setStakedAmount, setValicator, tokenPrice }) => {
  ...
  const onCreateStakingState = async () => {
    try {
      let txHash = await createStakingState(wallet, tokenMint, STAKING_IDS[1]);
    } catch (e) {
      console.error(e);
    }
  };
  ...
  useEffect(() => {
    const updateTVLAndScheduleUpdate = async () => {
      try {
        // Update Pool Status
        const flag = await getPoolStatus(wallet, tokenMint, STAKING_IDS[1]);
        setPoolStatus(+flag);

        // Update TVL
        const tvl = await getPoolAmount(wallet, tokenMint, STAKING_IDS[1]);
        setTVL(tvl);
  ...
  const onUpdatePool = async (e) => {
    ...
    try {
      let txHash = await updatePool(wallet, flag, tokenMint, STAKING_IDS[1]);
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const onStake = async () => {
    ...
    if (totalStake >= parseFloat(STAKING[1].LIMIT)) {
    ...
    try {
      setStaking(true);
      let txHash = await stake(wallet, stakeAmount, tokenMint, STAKING_IDS[1]);
    } catch (e) {
      console.error(e);
    } finally {
      setStaking(false);
  ...
  const onUnStake = async () => {
    ...
    try {
      setUnStaking(true);
      await unStake(wallet, unStakeAmount, tokenMint, STAKING_IDS[1]);
    } catch (e) {
  ...
  const onClaim = async () => {
    ...
    try {
      setClaimReward(true);
      await claim_staking(wallet, tokenMint, STAKING_IDS[1]);
    } catch (e) {
      showToast("Transaction failed", 2000, 1);
      console.error(e);
    } finally {
  ...
  return (
    <div className="flex flex-col border-[1px] border-[var(--borderColor)] p-4 gap-1 bg-[var(--cardColor)] rounded-2xl">
      ...
      <div className="flex justify-between items-center">
        ...
        <label className="font-bold text-[1.2rem] text-green-500">10%</label>
      </div>
      <div className="flex justify-between items-center">
