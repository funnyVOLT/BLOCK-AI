...
const Stake1Card = ({ setStakedAmount, setValicator, tokenPrice }) => {
  ...
  useEffect(() => {
    const updateTVLAndScheduleUpdate = async () => {
      try {
        // Update Pool Status
        const flag = await getPoolStatus(wallet, tokenMint, STAKING_IDS[0]);
        setPoolStatus(+flag);
        ...
        // const rewardAmount1 = await getRewardAmount(wallet, tokenMint, STAKING_IDS[0]);
        // const rewardAmount2 = await getRewardAmount(wallet, tokenMint, STAKING_IDS[1]);
        // const rewardAmount3 = await getRewardAmount(wallet, tokenMint, STAKING_IDS[2]);
        // const poolAmount1 = await getPoolAmount(wallet, tokenMint, STAKING_IDS[0]);
        // const poolAmount2 = await getPoolAmount(wallet, tokenMint, STAKING_IDS[1]);
        // const poolAmount3 = await getPoolAmount(wallet, tokenMint, STAKING_IDS[2]);
        // console.log('Total reward : ', rewardAmount1, rewardAmount2, rewardAmount3);
        // console.log('Total pool : ', poolAmount1, poolAmount2, poolAmount3);
  ...
  const onUpdatePool = async (e) => {
    ...
    try {
      let txHash = await updatePool(wallet, flag, tokenMint, STAKING_IDS[0]);
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const onStake = async () => {
    ...
    if (totalStake >= parseFloat(STAKING[0].LIMIT)) {
    ...
    try {
      setStaking(true);
      let txHash = await stake(wallet, stakeAmount, tokenMint, STAKING_IDS[0]);
    } catch (e) {
      console.error(e);
    } finally {
      setStaking(false);
  ...
  const onUnStake = async () => {
    ...
    try {
      setUnStaking(true);
      await unStake(wallet, unStakeAmount, tokenMint, STAKING_IDS[0]);
    } catch (e) {
  ...
  return (
    <div className="flex flex-col border-[1px] border-[var(--borderColor)] p-4 gap-1 bg-[var(--cardColor)] rounded-2xl">
      ...
      <div className="flex justify-between items-center">
        ...
        <label className="font-bold text-[1.2rem] text-green-500">8%</label>
      </div>
      <div className="flex justify-between items-center">
        <label className="font-bold">Pool Size</label>
        <label className="text-[0.9rem]">
          <span className="text-[1rem]">20000000</span> BLOCK
        </label>
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        {isAdminConnected() ? (
          <div className="flex flex-col gap-2 justify-center bg-gray-400 p-2 rounded-md">
            ...
            <Button onClick={onCreateStakingState}>Create Stake</Button>
